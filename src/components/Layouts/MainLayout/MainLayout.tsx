...
import { EnvStyle } from '@/styles/env'
import AmpState from '@/types/AmpState';
import Footer from '@/components/Footer/Footer';
import {PHONES_BY_COUNTRY, PhonesKey} from '@/const/general.constants';
import GetInTouch from '@/components/Modals/GetInTouch/GetInTouch';
import Success from '@/components/Modals/GetInTouch/Success';
...
export default function MainLayout(props: MainLayoutProps) {
  const { children, isServicePage, geo } = props;
  const slug: string = usePathname();
  const realSlug: string = slug.split('/')[1];
    ...
    <>
      <AmpState id="ui">
        <script
          ...
          dangerouslySetInnerHTML={{__html: JSON.stringify({menuOpened: false, nav: realSlug, modalOpen: false})}}
        />
      </AmpState>
      <EnvStyle/>
      <div
        ...
        data-amp-bind-class={'"off-canvas" + (ui.menuOpened ? " -active " : "")'}
      >
        <div className="is-drilldown">
          <ul className="mobile-nav-menu" role="menubar">
            <li
              className="menu-item nav-item dropdown"
              role="menuitem"
            ...
            >
              ...
              <div className="dropdown-menu">
                <a href={`/services/public-relations`} className="dropdown-item">
                  Public relations
                </a>
                <a href={`/services/pr-release`} className="dropdown-item">
                  PR release
                </a>
              </div>
            </li>
          ...
          <div className="mobile-btn__container flex-column">
            <div className="mobile-btn__container_row d-lg-flex">
              <button
                id="get_in_touch_mobile"
                className="header-btn btn get_in_touch_mobile"
            ...
            <div className="mobile-btn__container_row d-lg-flex">
              ...
              <a
                ...
                className="mobile-btn btn -secondary go_to_booking_mobile"
                aria-label="Book now"
                on={
                  isServicePage
                    ? "tap:AMP.setState({ ui: { menuOpened: false } }), booking_service.scrollTo(duration=400)"
                    : "tap:AMP.setState({ ui: { menuOpened: false } }), booking.scrollTo(duration=400)"
                }
              >
      ...
      <div
        ...
        role="banner"
        tabIndex={0}
        on="tap:AMP.setState({ ui: { menuOpened: false } })"
        data-amp-bind-class={'"off-canvas-overlay" + (ui.menuOpened ? " -active " : "")'}
      />

      <Header geo={geo} slug={realSlug} isServicePage={isServicePage} />


      <div
        id="pageContent"
        className="off-canvas-content"
        data-amp-bind-class={'"off-canvas-content" + (ui.menuOpened ? " -active " : "")'}
      >
